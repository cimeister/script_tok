...
def _load_text_stats(path, required=True):
    ...
    if not os.path.exists(path):
        if required:
            raise SystemExit(
                f"text stats not found: {path}\n"
                f"Regenerate with measure_text_stats.py, or pass --allow-missing."
            )
        return {}
    return json.load(open(path)).get("arms", {})


def _load_results(path, required=True):
    """results.tsv rows grouped by arm."""
    if not os.path.exists(path):
        # Returning {} here would emit "no downstream runs finished yet" and silently
        # replace a table of real results with a placeholder.
        if required:
            raise SystemExit(
                f"results not found: {path}\n"
                f"Pass --results, or --allow-missing to emit a placeholder table on purpose."
            )
        return {}
...
def main(
    ...
    results: str = os.path.join(HERE, "results.tsv"),
    text_stats: str = os.path.join(HERE, "text_stats.json"),
    out: str = DEFAULT_OUT,
    ...
    Args:
        ...
        results: TSV from collect_results.py. Defaults to the committed snapshot beside this
            script, so a clean clone regenerates the paper's tables. During a sweep, point it
    ...
    res = _load_results(results, required=not allow_missing)
    stats = _load_text_stats(text_stats, required=not allow_missing)
    ...
    body = [
        ...
        "% sources: "
        + ", ".join(os.path.relpath(p, REPO) for p in (manifest, results, text_stats)),
        "",
        r"\begin{table}[t]",
        r"\centering",
        r"\small",
